...
async function crearPerfiles() {
  ...
  // Deshabilitar RLS
  const { error: disableError } = await supabase.rpc('disable_rls_for_migration');
  if (disableError) {
    console.log("ℹ️  RLS probablemente ya está deshabilitada o no existe la función");
  }

  // Directamente vía SQL
  try {
    const { error: rls } = await supabase
      .from('profiles')
